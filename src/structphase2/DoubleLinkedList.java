...
public class DoubleLinkedList<T> {
	...
	public void add(Brand x, int index) {
		if (index == 0)
			addFisrt(x);
		else {
			if (index >= getCount())
	...
	public int search(String brand) {
		int index = 0;
		DoubleNode<T> current = first;
		for (int i = 0; i < getCount(); i++) {
			if (((Brand) current.getElement()).getBrand().equals(brand)) {
				return index;
			}
			index++;
			current = current.next;
